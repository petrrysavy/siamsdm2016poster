...
\block{Hamerly's algorithm \cite{hamerly}}{
  \begin{itemize}
    ...
    \item One \emph{lower bound} $l(\vec{x})$ on the distance
...
\block{Tighter update}{
    \cite{elkan, hamerly} assume the worst case while
    they update the upper/lower bounds.
    Points in cluster are not everywhere in space, but
    they fulfill some locality. When a centroid $\cj$
    moves away from a point $\x$, the lower bound $\lxcj$
    does not have to shrink. If the lower bound decreases
    slower, the distance should be recalculated less
    often.
