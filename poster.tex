...
\block{Triangle-inequality-based $k$-means}{
   ...
   target this issue by maintaining a set of upper and lower bounds.
   Those bounds are based on the triangle inequality.   
...
\block{Tighter update}{
    ...
    they fulfill some locality. When a centroid moves away,
    the lower bound does not have to shrink.
...
\block{Avoiding $\Theta(n)$ work in the innermost loop}{
    ...
    the second closest neighbor in the innermost loop.
    What if we knew what was the second closest centroid?
    If we know the closest and the second closest centroid,
    we do not need to calculate the distance to the other centroids.
  \begin{definition}
  ...
  \end{equation}
    ...
    we know that for $\x$ assigned to $\ci$:
    \begin{itemize}
...
\block{Experimental results}{
 ...
 Changes require operations per iteration and pair of centroids. There is no additional work per point. Therefore the changes introduce a low risk of slowing down the algorithm while good chances of improving the runtime. Changes work better in lower dimension and when data are clustered. Algorithms becomes up to 8 times faster that the original versions.
}
